...
// TODO:
//  - PARA ESTA OPERACIÓN CREO QUE SE PODRÍA OPTIMIZAR SI SE EJECUTA AL MISMO TIEMPO QUE SE LEA EL
//  ARCHIVO
...
//  HABRÁ UN ARRAY QUE CONTENGA TODOS LOS PÍXELES, CADA POSICIÓN DEL ARRAY TENDRÁ TRES CAMPOS (R G Y
//  B)
// PSEUDOCODE
// RECIBE UN ARGUMENTO:
...
double distancia_euclidiana(Color const & c1, Color const & c2) {
  return std::sqrt(std::pow(c1.red - c2.red, 2) + std::pow(c1.green - c2.green, 2) +
                   std::pow(c1.blue - c2.blue, 2));
}

// FUNCIÓN PARA CONTAR LA FRECUENCIA DE CADA COLOR
// Un unordered_map es como un diccionario pero su tiempo de búsqueda es O(1), y si se repite la
// clave, se sobreescribe Paso el vector por referencia para no hacer una copia y así ser más
// eficiente
std::unordered_map<Color, int> contar_frecuencia(std::vector<Color> const & pixeles) {
  std::unordered_map<Color, int> frecuencia;

  for (auto const & pixel : pixeles) { frecuencia[pixel]++; }
...
// Toma como parámetro de entrada el unordered map (llamado frecuencia) y el número de colores a
// seleccionar, y retorna el vector de los n colores menos frecuentes

std::vector<Color>
    encontrar_colores_menos_frecuentes(std::unordered_map<Color, int> const & frecuencia,
                                       long unsigned int n) {
